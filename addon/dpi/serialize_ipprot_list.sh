...
egrep -v "^#" /etc/protocols | egrep -v "^ip\s+0\s+IP" | egrep -v "^(\s+)?$" | awk '{print "  " $2 ": \"" $3 "\","}'
echo -e "  255: \"UNKNOWN\",\n}\n\nfunc GetIPProto(id int) string {\n  return IPProtocols[id]\n}\n"

echo -e "\nvar IPProtocolIDs = map[string] int {"
egrep -v "^#" /etc/protocols | egrep -v "^ip\s+0\s+IP" | egrep -v "^(\s+)?$" | grep -v "for experimentation and testing" | awk '{print "  \"" $3 "\": " $2 ","}'
